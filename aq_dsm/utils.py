...
def create_prompt(model_data, change_request):
    ...
    ```{model_data}```
    The change request:`{change_request}`. List the components in the system that could be influenced by that change with the reasoning. Please, be specific! Please, provide only the elements with type Components and Sub-system and System types If they have no children. Please, leave the names of the componetns as they presented in the model. Please, provide at first the components that directly affected by that change, and then components affected by the change of the first set. So, in the intermediate answer I expect 1st level of change and then 2nd level of change caused by 1st level changes. After that provide the final output with format provided in Final Output Example with ONLY 1st level of changes
...
def to_the_calc(df, cols = ["Component", "Change"]):
    df_calc = df[cols]
    return df_calc.to_csv(os.path.join("outputs", "to-the-calc.csv"), index = False)
